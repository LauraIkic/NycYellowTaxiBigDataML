...
def create_app():
    ...
    # Load and prepare data - NOW WITH ALL 9 DATAFRAMES
    (df_monthly, df_weekday, df_heatmap, df_fare_weekday,
     df_hourly, df_top_pickup, df_payment,
     df_weather_conditions, df_weather_scatter) = load_data()
    ...
    # Dashboard Layout
    app.layout = html.Div([
        html.H1("NYC Yellow Taxi Dashboard",
                style={'textAlign': 'center', 'marginBottom': 30, 'color': '#FFD700'}),
        ...
        html.Div([
            html.Div([dcc.Graph(id='weather-condition-chart')],
                     style={'width': '48%', 'display': 'inline-block', 'marginRight': '2%'}),
            html.Div([dcc.Graph(id='weather-scatter-chart')],
                     style={'width': '48%', 'display': 'inline-block'}),
        ]),
    ...
    def update_heatmap_chart(_):
        ...
        fig.update_layout(**create_chart_layout(
            'Trip Heatmap: Day of Week vs Month',
            'Month',
            'Day of Week',
            500
        ))
    ...
    def update_fare_weekday_chart(_):
        ...
        if not df_fare_weekday.empty:
            ...
            for i, (day, revenue) in enumerate(zip(df_fare_weekday['day_name'],
                                                   df_fare_weekday['total_revenue'])):
                fig.add_annotation(
                    x=day,
                    y=revenue,
                    text=f"${revenue:,.0f}",
    ...
    def update_monthly_trips_chart(_):
        ...
        if not df_monthly.empty:
            ...
            for i, (date, count) in enumerate(zip(df_monthly['year_month'],
    ...
    def update_payment_type_chart(_):
        ...
        if not df_payment.empty:
            ...
            df_payment_copy['payment_name'] = df_payment_copy['payment_type'].map(
                payment_type_names
            ).fillna('Other')
    ...
    @callback(Output('weather-condition-chart', 'figure'),
              Input('weather-condition-chart', 'id'))
    ...
    @callback(Output('weather-scatter-chart', 'figure'),
              Input('weather-scatter-chart', 'id'))
    def update_weather_scatter_chart(_):
        ...
        if not df_weather_scatter.empty:
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    size=10,
                    color=df_weather_scatter['trip_count'],
                    colorscale='Viridis',
                    showscale=True
                ),
